...
class NavigationNode(Node):
    def __init__(self):
        ...
        goal_pose_0 = self.create_pose(-0.12, -2.35, 3.14)
        goal_pose_1 = self.create_pose(1.86, 2.56, 0.97)
        goal_pose_2 = self.create_pose(-3.84, 2.64, 2.78)

        self.navigator.setInitialPose(init_pose)
        self.navigator.waitUntilNav2Active()

        # Execute navigation tasks
        if self.navigate_to_pose(goal_pose_0):
            time.sleep(2)
            if self.navigate_to_pose(goal_pose_1):
                time.sleep(2)
                self.navigate_to_pose(goal_pose_2)
                time.sleep(2)
